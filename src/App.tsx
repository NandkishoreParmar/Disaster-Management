...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'disasters' | 'social' | 'resources' | 'updates'>('disasters');
  ...
  const loadDisasterData = async (disasterId: string) => {
    ...
    try {
      ...
      const socialResponse = await fetch(`http://localhost:3001/api/social-media/${disasterId}/social-media`, {
      ...
      // Load resources
      const resourcesResponse = await fetch(`http://localhost:3001/api/resources/${disasterId}/resources`, {
        headers: { 'X-User-ID': 'netrunnerX' }
      });
      ...
      const updatesResponse = await fetch(`http://localhost:3001/api/updates/${disasterId}/official-updates`, {
        headers: { 'X-User-ID': 'netrunnerX' }
      });
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500 mt-2 line-clamp-2">{disaster.description}</p>
          ...
          <div className="lg:col-span-2 space-y-6">
            {selectedDisaster ? (
              <>
                ...
                <div className="bg-white rounded-lg shadow-md">
                  ...
                  <div className="p-6">
                    {activeTab === 'social' && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium text-gray-900">Social Media Reports</h3>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <div className="flex items-center space-x-3 mb-2">
                                  ...
                                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getUrgencyColor(post.urgency)}`}>
                                    {post.urgency}
                                  </span>
                                  {post.verified && (
                    ...
                    {activeTab === 'resources' && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium text-gray-900">Official Resources</h3>
                        {officialUpdates.map((update) => (
                          <div key={update.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                            ...
                            <p className="text-sm text-gray-600 mb-2">{update.source}</p>
                    ...
                    {activeTab === 'updates' && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium text-gray-900">Official Updates</h3>
                        {officialUpdates.map((update) => (
                          <div key={update.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                            <div className="flex items-start justify-between mb-2">
                              <h4 className="font-medium text-gray-900">{update.title}</h4>
              ...
              <div className="bg-white rounded-lg shadow-md p-12 text-center">
                ...
                <p className="text-gray-600">Choose a disaster from the list to view detailed information and real-time updates.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">Report New Disaster</h3>
            </div>
